...
void doLowering () {
    ...
    if (do_generate_omp_parallel) {
        generate_omp_parallel();
    }
    ...
    if (main_tensors_rhs.size() == 1) { // Generate "a = b"
        if (ConstantOp cstop = dyn_cast<ConstantOp>(main_tensors_rhs[0].getDefiningOp())) { // "a = 1.0"
            if (comp_worksp_opt) { // true attr means compressed workspace
                /// ----------------- ///
                /// Previous WS needs this, generating the 1st compute node, but this should be deprecated.
                /// ----------------- ///
                ...
                genCmptOpGeneralInitialAssignment
            }
