...
func.func @main() {
  ...
  ^bb0(%arg0: !it.symbolic_domain<64>):
    ...
    %58 = scf.forall (%arg1) in (%21) shared_outs(%arg2 = %57#4) -> (tensor<?xi64>) {
      ...
      %row_id_plus_one = arith.addi %arg1, %c1 : index
      %extracted_slice_arg = tensor.extract_slice %arg2[%row_id_plus_one] [%c1] [1] : tensor<?xi64> to tensor<?xi64>
      %63 = builtin.unrealized_conversion_cast %arg1, %57#1, %c0_46, %57#3, %extracted_slice_arg, %57#5 : index, index, index, index, tensor<?xi64>, tensor<?xi64> to !it.symbolic_domain<64>
      ...
      /// SymbolicDomainEndRowOp and SymbolicDomain explain:
      /// 1. symbolic_domain#0: row_id, and mark_value base
      /// 2. symbolic_domain#2: crd_size
      /// 3. symbolic_domain#4: pos tensor
      /// Then, what SymbolicDomainEndRowOp does is to set
      /// symbolic_domain#4[symbolic_domain#0+1] = symbolic_domain#2

      // %68 = "it.SymbolicDomainEndRowOp"(%67) <{needs_mark = true}> : (!it.symbolic_domain<64>) -> !it.symbolic_domain<64>
      // %69:6 = builtin.unrealized_conversion_cast %68 : !it.symbolic_domain<64> to index, index, index, index, tensor<?xi64>, tensor<?xi64>
      // %extracted_slice = tensor.extract_slice %69#4[%c0] [%c1] [1] : tensor<?xi64> to tensor<?xi64>
      // scf.forall.in_parallel {
      //   tensor.parallel_insert_slice %extracted_slice into %arg2[%row_id_plus_one] [%c1] [1] : tensor<?xi64> into tensor<?xi64>
      // }

      /// version 1
      %out_symbolic_domain:6 = builtin.unrealized_conversion_cast %67 : !it.symbolic_domain<64> to index, index, index, index, tensor<?xi64>, tensor<?xi64>
      %crd_size = arith.index_cast %out_symbolic_domain#2 : index to i64
      %inserted = tensor.insert %crd_size into %out_symbolic_domain#4[%c0] : tensor<?xi64>
      scf.forall.in_parallel {
        tensor.parallel_insert_slice %inserted into %arg2[%row_id_plus_one] [%c1] [1] : tensor<?xi64> into tensor<?xi64>
      }
