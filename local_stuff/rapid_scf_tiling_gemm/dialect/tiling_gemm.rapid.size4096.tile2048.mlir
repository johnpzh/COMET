...
func.func @main() {
  ...
  %A1 = arith.constant 4096 : index
  %A2 = arith.constant 4096 : index
  %B1 = arith.constant 4096 : index
  %B2 = arith.constant 4096 : index
  %C1 = arith.constant 4096 : index
  %C2 = arith.constant 4096 : index
  %A_size = arith.muli %A1, %A2 : index
  %B_size = arith.muli %B1, %B2 : index
  %C_size = arith.muli %C1, %C2 : index
  %A1_tile = arith.constant 2048 : index
  %A2_tile = arith.constant 2048 : index
  %B1_tile = arith.constant 2048 : index
  %B2_tile = arith.constant 2048 : index
  %A_tile_size = arith.muli %A1_tile, %A2_tile : index
  %B_tile_size = arith.muli %B1_tile, %B2_tile : index



  %A_matrix = memref.alloc() {allocator = "rapid"} : memref<4096x4096xf64>
  %B_matrix = memref.alloc() {allocator = "rapid"} : memref<4096x4096xf64>
  %C_matrix = memref.alloc() {allocator = "rapid"} : memref<4096x4096xf64>
  // %A_matrix = memref.alloc() : memref<4096x4096xf64>
  // %B_matrix = memref.alloc() : memref<4096x4096xf64>
  // %C_matrix = memref.alloc() : memref<4096x4096xf64>
  scf.for %i = %c0 to %A1 step %c1 {
    scf.for %j = %c0 to %A2 step %c1 {
      memref.store %cst1, %A_matrix[%i, %j] : memref<4096x4096xf64>
    }
  }
  scf.for %i = %c0 to %B1 step %c1 {
    scf.for %j = %c0 to %B2 step %c1 {
      memref.store %cst2, %B_matrix[%i, %j] : memref<4096x4096xf64>
    }
  }
  scf.for %i = %c0 to %C1 step %c1 {
    scf.for %j = %c0 to %C2 step %c1 {
      memref.store %cst0, %C_matrix[%i, %j] : memref<4096x4096xf64>
    }
  }
      ...
      scf.for %jj = %c0 to %B2 step %B2_tile {
        ...
        scf.for %i = %ii to %i_bound step %c1 {
          scf.for %k = %kk to %k_bound step %c1 {
            %A_val = memref.load %A_matrix[%i, %k] : memref<4096x4096xf64>
            scf.for %j = %jj to %j_bound step %c1 {
              %B_val = memref.load %B_matrix[%k, %j] : memref<4096x4096xf64>
              %mul_res = arith.mulf %A_val, %B_val : f64
              %old_val = memref.load %C_matrix[%i, %j] : memref<4096x4096xf64>
              %new_val = arith.addf %old_val, %mul_res : f64
              memref.store %new_val, %C_matrix[%i, %j] : memref<4096x4096xf64>
            }
          }
        }
      }
  ...
  // %C_cast = memref.cast %C_matrix : memref<4096x4096xf64> to memref<*xf64>
  ...
  scf.for %i = %c0 to %C1 step %c1 {
    scf.for %j = %c0 to %C2 step %c1 {
      %C_val = memref.load %C_matrix[%i, %j] : memref<4096x4096xf64>
  ...
  // memref.dealloc %A_matrix : memref<4096x4096xf64>
  // memref.dealloc %B_matrix : memref<4096x4096xf64>
  // memref.dealloc %C_matrix : memref<4096x4096xf64>
