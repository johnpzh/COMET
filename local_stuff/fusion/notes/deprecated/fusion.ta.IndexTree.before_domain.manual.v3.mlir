...
module {
  func.func @main() {
    ...
    %T = "ta.dense_tensor_decl"(%5) <{format = "Dense"}> : (index) -> tensor<?xf64>
    ...
    "ta.fill"(%T) <{value = 0.000000e+00 : f64}> : (tensor<?xf64>) -> ()
    // "ta.fill"(%T) <{value = 0.000000e+00 : f64}> : (tensor<?xf64>) -> ()
    // %11:2 = "it.itree"(%T) ({
    %11 = "it.itree"(%T) ({
    // %11 = "it.itree"(%10) ({
      ...
      %crd, %pos = "it.IndexToTensorDim"(%T, %15) <{dim = 0 : ui32}> : (tensor<?xf64>, !it.index) -> (index, index)  /// %T = T, %15 = i
      // %crd_0, %pos_1 = "it.IndexToTensorDim"(%T, %14, %pos) <{dim = 1 : ui32}> : (tensor<?x4xf64>, !it.index, index) -> (index, index)  /// %14 = h
      // %17 = "it.LHSOperandOp"(%T, %pos, %pos_1, %crd, %crd_0) : (tensor<?x4xf64>, index, index, index, index) -> !it.operand
      ...
      %20 = "it.ComputeOp"(%16, %17, %18, %19) <{compute_missing = false, operandSegmentSizes = array<i32: 1, 1, 2, 0>, semiring = "plusxy_times"}> : (!it.index, !it.operand, !it.operand, !it.operand) -> tensor<?xf64>  /// T = B * C

      
      %dcrd_10, %dpos_11 = "it.IndexToTensorDim"(%20, %15) <{dim = 0 : ui32}> : (tensor<?xf64>, !it.index) -> (index, index)  /// %6 = T, %15 = i
      // %dcrd_10, %dpos_11 = "it.IndexToTensorDim"(%21, %d17) <{dim = 0 : ui32}> : (tensor<?xf64>, !it.index) -> (index, index)  /// %6 = T, %15 = i
      %d22 = "it.LHSOperandOp"(%20, %dpos_11, %dcrd_10) : (tensor<?xf64>, index, index) -> !it.operand  /// %6 = T
      %cst = arith.constant 0.000000e+00 : f64
      %d23 = "it.OperandOp"(%cst) : (f64) -> !it.operand  /// 0.0, or Use a 1D tensor
      %d24 = "it.ComputeOp"(%15, %d22, %d23) <{compute_missing = false, operandSegmentSizes = array<i32: 1, 1, 1, 0>, semiring = "noop_times"}> : (!it.index, !it.operand, !it.operand) -> tensor<?xf64>

    //   it.yield %20, %d24 : tensor<?xf64>, tensor<?xf64>
    // }) : (tensor<?xf64>) -> (tensor<?xf64>, tensor<?xf64>)
    // "ta.set_op"(%11#0, %T) {__beta__ = 0.000000e+00 : f64} : (tensor<?xf64>, tensor<?xf64>) -> ()
      it.yield %20 : tensor<?xf64>
    }) : (tensor<?xf64>) -> (tensor<?xf64>)
    "ta.set_op"(%11, %T) {__beta__ = 0.000000e+00 : f64} : (tensor<?xf64>, tensor<?xf64>) -> ()
    "ta.print"(%T) : (tensor<?xf64>) -> ()
    return
  }
}