...
module {
  func.func @main() {
    %0 = "ta.dense_tensor_decl"() <{format = "Dense"}> : () -> tensor<8x4xf64>  /// %0 = A
    %1 = "ta.dense_tensor_decl"() <{format = "Dense"}> : () -> tensor<4x2xf64>  /// %1 = B
    %2 = "ta.dense_tensor_decl"() <{format = "Dense"}> : () -> tensor<8x2xf64>  /// %2 = C
    ...
    %3 = "it.itree"() ({
      ...
      %5 = "it.DomainOp"(%0) <{dim = 0 : ui32, format = 0 : i32}> : (tensor<8x4xf64>) -> !it.domain  /// %0 = A, %5 = i
      %6 = "it.IndexOp"(%4, %5) : (!it.index_tree, !it.domain) -> !it.index  /// %6 = i
      %7 = "it.DomainOp"(%1) <{dim = 0 : ui32, format = 0 : i32}> : (tensor<4x2xf64>) -> !it.domain  /// %1 = B, %7 = j
      %8 = "it.DomainOp"(%0) <{dim = 1 : ui32, format = 0 : i32}> : (tensor<8x4xf64>) -> !it.domain  /// %0 = A, %8 = j
      %9 = "it.DomainIntersectionOp"(%8, %7) <{operandSegmentSizes = array<i32: 2, 0>}> : (!it.domain, !it.domain) -> !it.domain  /// %9 = j
      %10 = "it.IndexOp"(%6, %9) : (!it.index, !it.domain) -> !it.index  /// %10 = j
      %11 = "it.DomainOp"(%1) <{dim = 1 : ui32, format = 0 : i32}> : (tensor<4x2xf64>) -> !it.domain  /// %1 = B, %11 = k
      %12 = "it.IndexOp"(%10, %11) : (!it.index, !it.domain) -> !it.index  /// %12 = k
      ...
      %16 = "it.ComputeOp"(%12, %13, %14, %15) <{compute_missing = false, semiring = "plusxy_times"}> : (!it.index, !it.operand, !it.operand, !it.operand) -> tensor<8x2xf64>
